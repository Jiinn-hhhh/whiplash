...
Manager는 `claude -p --resume`으로 에이전트 인스턴스를 직접 생성·관리한다. 두 가지 실행 모드를 지원한다:

| 모드 | 설명 |
|------|------|
| **단독 (solo)** | 역할별 에이전트 1개씩 실행 |
| **멀티 (dual)** | 같은 태스크를 두 백엔드(Claude Code + Codex)에서 이중 실행 → Manager가 중재하여 합의 도출 |

실행 모드는 프로젝트 온보딩 시 유저가 선택한다. 상세: `agents/manager/techniques/orchestration.md`
