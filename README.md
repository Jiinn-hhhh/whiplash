...
| **Systems Engineer** | live 시스템, 배포 경로, 런타임, 클라우드, 드리프트 분석 | opus | 전체 |
...
- Developer와 Systems Engineer만 Write/Edit 도구를 가진다
...
|------|------|-----------------|------|
| **solo** | Manager가 역할별 에이전트를 하나씩 실행 (tmux 기반) | `manager`, `developer`, `researcher`, `monitoring`, 필요 시 `systems-engineer` | 1x |
| **dual** (실험적) | 같은 태스크를 Claude Code + Codex CLI 이중 실행, Manager가 합의 도출 | `manager`, `developer-claude`, `developer-codex`, `researcher-claude`, `researcher-codex`, `monitoring`, 필요 시 `systems-engineer`(solo) | 2x |
...
- 새 프로젝트에서 `project.md`가 아직 없으면 `boot-onboarding`이 bootstrap 초안과 기본 디렉토리를 자동 생성한다. 이 초안의 `실행 모드`와 `활성 에이전트`는 온보딩 과정에서 확정한다.
...
tmux 세션: whiplash-{project}
  ...
  ├─ [6] systems-engineer      ← 프로젝트에 따라 활성
  ├─ [7] monitoring
  └─ [8] researcher-2          ← 동적 스폰 (필요 시)
```
...
5. Manager가 준비되면 `cmd.sh boot {project}`가 정식 팀을 부팅한다.
