...
- [fswatch](https://emcrisostomo.github.io/fswatch/) — 선택, mailbox 실시간 감시 (`brew install fswatch`). 없으면 5초 폴링 자동 전환.
...
- **mailbox (실시간 알림)**:
  - 에이전트 A → `mailbox.sh` → `mailbox/{role-B}/new/` 파일 생성
  - `monitor.sh`가 fswatch로 감지 → tmux send-keys로 Agent B에 push
  - 메시지 전달 후 즉시 삭제
  - 종류: task_complete, status_update, need_input, escalation, agent_ready
  - 감사 로그: `memory/manager/logs/mailbox-audit.log`
...
- 에이전트 완료 → mailbox로 task_complete 보고
- dual 모드: 양쪽 실행 → Manager가 합의 도출

### 6. 장애 대응

- **크래시 감지**: monitor.sh 30초 헬스체크 → tmux 윈도우 소멸 감지 → 자동 reboot (최대 3회)
- **행(hung) 감지**: 10분 비활성 → Manager에게 1회 알림 (자동 kill 안 함)
- **monitor.sh 자기 복구**: Manager가 `monitor-check`로 PID+heartbeat 확인 → 죽었으면 재시작
- **세션 리프레시**: 맥락 과다 시 Manager가 수동 실행 가능 (auto-compact가 대부분 처리)

### 7. 종료

- Manager가 `orchestrator.sh shutdown {project}` 실행
- 모든 에이전트 세션 종료, tmux 세션 삭제, monitor.sh PID kill
