...
Manager가 태스크 지시서를 작성하고 `dispatch`/`dual-dispatch`로 전달. 에이전트는 완료 후 `message.sh`로 `task_complete` 보고.
...
6. **확정**: 합의 내용을 `memory/manager/`에 기록, 결과를 정식 배치
...
- 크로스 프로젝트 참조는 명시적 전체 경로: `projects/{other}/memory/...`
