...
```
유저 대화 → Onboarding이 프로젝트 설계 → Manager가 팀 자동 구성 → 에이전트들이 작업
```
...
- Developer만 Write/Edit 도구를 가지므로 프로덕션 코드 수정 권한이 Developer에게 제한됨
...
|------|------|-----------------|------|
| **solo** | Manager가 역할별 에이전트를 하나씩 실행 (tmux 기반) | `manager`, `developer`, `researcher`, `monitoring` | 1x |
| **dual** (실험적) | 같은 태스크를 Claude Code + Codex CLI 이중 실행, Manager가 합의 도출 | `manager`, `developer-claude`, `developer-codex`, `researcher-claude`, `researcher-codex`, `monitoring` | 2x |
...
tmux 세션: whiplash-{project}
  ...
  ├─ [1] manager
  ├─ [2] developer(-claude)
  ├─ [3] developer-codex       ← dual 모드만
  ├─ [4] researcher(-claude)
  ├─ [5] researcher-codex      ← dual 모드만
  ├─ [6] monitoring
  └─ [7] researcher-2          ← 동적 스폰 (필요 시)
```
...
```
유저 ──→ Onboarding ──→ cmd.sh boot-manager ──→ Manager tmux 세션 생성
                                                    │
                                Manager 자동 실행:  cmd.sh boot {project}
                                                    │
                            ┌───────────┬───────────┼───────────┐
                            ↓           ↓           ↓           ↓
...
1. Onboarding이 `project.md` 작성 완료 후 `cmd.sh boot-manager`로 Manager 부팅
2. Manager가 온보딩(Layer 1 문서 읽기) 완료 후 `cmd.sh boot`로 팀 부팅
3. `preflight.sh`가 의존성 검증 (tmux, jq, python3, codex 등)
4. 각 에이전트: `claude -p` → session_id 획득 → tmux 윈도우 → `claude --resume`
5. 모든 에이전트가 `agent_ready` 알림 → Manager가 첫 태스크 분배
...
Manager 에이전트가 내부적으로 실행한다. 유저는 `boot-manager`와 `shutdown`만 직접 사용.

```bash
# 유저 실행
cmd.sh boot-manager   {project}                   # Manager 부팅 (진입점)
...
| 역할 기반 파일 접근 | 프로젝트 코드는 Developer만 수정. 도구 제한(--allowedTools)으로 강제 |
