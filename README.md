...
AI 코딩 에이전트(Claude Code 등)를 이 폴더에서 실행하고, 아래 명령을 내린다:
...
1. `orchestrator.sh boot-manager`가 실행되어 Manager를 tmux 세션에 부팅한다
2. 유저에게 `tmux attach -t whiplash-{project}` 접속 안내가 뜬다
3. 접속하면 Manager가 이미 준비되어 있다
4. Manager에게 지시하면 나머지 에이전트를 부팅하고 팀을 운영한다
...
이후에는 Manager와 대화하며 프로젝트를 진행한다. 유저는 가끔 중요한 결정만 내리면 된다.
