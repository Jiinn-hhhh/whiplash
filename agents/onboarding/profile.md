...
- 유저와 `실행 프리셋(default/claude-only/codex-only/dual)`, 역할별 backend/model baseline, `작업 루프(guided/ralph)` 를 구분해 결정
- `ralph`를 선택한 경우 유저가 직접 `랄프 완료 기준`과 `랄프 종료 방식`을 적도록 유도
...
- `ralph` 종료 정책을 onboarding이 대신 정하지 않는다 — 유저가 직접 정한다
