...
```bash
bash scripts/orchestrator.sh boot {project-name}
```
...
9. 태스크 완료 또는 블로커 발생 시:
   bash scripts/notify.sh {project} {role} manager {kind} {priority} "{subject}" "{content}"
...
# 2. Researcher에게 디스패치
bash scripts/orchestrator.sh dispatch researcher \
...
bash scripts/orchestrator.sh dispatch developer \
...
```bash
bash scripts/orchestrator.sh dual-dispatch {role} {task-file} {project}
```
...
3. **교차 전달**: notify.sh로 양쪽 에이전트에 `consensus_request`를 보낸다.
   ```bash
   bash scripts/notify.sh {project} manager {role}-claude \
     consensus_request normal "합의 요청" "workspace/shared/discussions/DISC-NNN.md를 읽고 의견을 추가하라."
   bash scripts/notify.sh {project} manager {role}-codex \
     consensus_request normal "합의 요청" "workspace/shared/discussions/DISC-NNN.md를 읽고 의견을 추가하라."
   ```
...
**모델**: 모든 에이전트 opus. 단, Monitoring만 haiku (가성비).

**도구**: 모든 에이전트가 전체 도구(Read,Glob,Grep,Write,Edit,Bash,WebSearch,WebFetch)를 사용한다. 역할별 제한 없음. `--dangerously-skip-permissions`로 도구 승인을 바이패스한다.
...
| 파일 | 내용 | 기록 주체 |
|------|------|-----------|
| `system.log` | 인프라 이벤트 (부팅/종료/크래시/디스패치 등) | orchestrator.sh, monitor.sh |
| `message.log` | 에이전트 간 메시지 전달 이력 | notify.sh |
...
bash scripts/notify.sh {project} researcher manager \
  task_complete normal "TASK-001 완료" "리서치 결과를 workspace/teams/research/에 작성함"

# 에이전트가 다른 에이전트에게 직접 알림
bash scripts/notify.sh {project} researcher developer \
...
```bash
# 상태 확인
bash scripts/orchestrator.sh status {project}

# monitor.sh 상태 확인 + 자동 재시작
bash scripts/orchestrator.sh monitor-check {project}
...
```bash
bash scripts/orchestrator.sh reboot {role} {project}
```
...
```bash
bash scripts/orchestrator.sh refresh {role} {project}
```
...
```bash
bash scripts/orchestrator.sh spawn {role} {window-name} {project} [extra-msg]
# 예시: researcher가 바쁠 때 researcher-2 추가
bash scripts/orchestrator.sh spawn researcher researcher-2 myproject
# 예시: 핫픽스 전용 developer
bash scripts/orchestrator.sh spawn developer dev-hotfix myproject "긴급 버그 수정 전용"
```

### 종료

```bash
bash scripts/orchestrator.sh kill-agent {window-name} {project}
# 예시
bash scripts/orchestrator.sh kill-agent researcher-2 myproject
```
...
|------|-----------|---------|
| 무인 실행 | --dangerously-skip-permissions | --yolo |
| --resume (세션 재개) | O | X |
| Hooks (.claude/) | O | X |
| notify.sh (Bash) | O | O |
| tmux send-keys | O | O |
