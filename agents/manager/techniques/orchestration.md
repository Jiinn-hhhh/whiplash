...
> **참고**: 이 문서의 모든 `cmd.sh`, `message.sh`, `monitor.sh` 명령은 Manager 에이전트가 내부적으로 실행한다. 단, 유저는 온보딩 흐름에서 `cmd.sh boot-onboarding`, `cmd.sh boot-manager`, `cmd.sh shutdown`을 직접 실행할 수 있다.
...
### Onboarding -> Manager 전환

- Onboarding이 설계를 마치면 내부적으로 `cmd.sh boot-manager {project-name}`를 실행한다.
- 이미 onboarding 세션이 떠 있으면 `boot-manager`가 같은 tmux 세션에서 Manager 부팅으로 이어진다.
- Onboarding이 정리한 `memory/onboarding/handoff.md`가 있으면 Manager가 먼저 읽는다.
- 전환이 끝나면 onboarding 분석용 윈도우는 정리되고, Manager가 `cmd.sh boot`로 정식 팀을 올린다.
...
2. `discussion` 세션 단일 부팅 (manager와 같은 backend 사용, dual에서도 solo 유지)
3. project.md에서 활성 에이전트 목록 추출
4. 각 에이전트에 대해 (`manager`, `discussion`은 건너뜀):
...
Dual 모드에서는 **Claude Code**와 **Codex CLI** 두 백엔드가 동시에 떠 있다. Manager는 태스크 성격에 따라 **이중 실행(비교/검증)** 또는 **분업(각자 다른 태스크)**을 유기적으로 판단한다.

- **고정 규칙이 아니라 Manager의 유기적 판단**이다. 아래 표는 참고 가이드일 뿐.
- 합의는 **반자동** — cmd.sh에 도구(`dual-dispatch` 등)만 제공하고, Manager가 문서화된 절차를 따라 직접 판정한다.
- Systems Engineer와 Monitoring 에이전트는 dual에서도 **solo로 부팅**한다 (기본 이중 실행 대상 아님).

#### 태스크 성격별 모드 선택 가이드

| 태스크 성격 | 모드 | 이유 |
|---|---|---|
| 설계/아키텍처/전략 | dual — 이중 실행(비교) | 판단 품질이 중요. 두 결과를 비교해서 더 나은 걸 고르는 가치가 있다 |
| 구현/수정/단순 작업 | 분업 — 각자 다른 태스크 | 속도가 중요. 같은 걸 둘이 할 이유 없다 |
| 리스크 높은 변경 | dual — 이중 실행(검증) | 실수 방지. 한 쪽이 놓친 걸 다른 쪽이 잡을 수 있다 |

#### 한 쪽이 먼저 끝났을 때

- **기다릴 가치가 있으면** (설계/전략/리스크): 다른 쪽 결과를 대기한다.
- **기다릴 가치가 없으면** (구현/단순): 끝난 쪽에 다른 태스크를 즉시 배부한다.
- Codex가 보통 더 빠른 경향이 있다는 점도 고려한다.

### 6.2 이중 부팅

`cmd.sh boot`가 project.md의 `실행 모드: dual`을 감지하면 자동으로 이중 부팅한다:

- dual 기본 대상 역할(`developer`, `researcher`)에 대해서만 두 윈도우 생성:
...
- **Developer와 Systems Engineer만 수정 가능**. 다른 에이전트는 읽기 전용.
- Developer는 제품 코드와 애플리케이션 구현을 담당한다.
- Systems Engineer는 운영 자동화, 배포 스크립트, 런북, 인프라 관련 코드를 담당한다.
...
- `memory/knowledge/docs/`: 역할별 stable reference 문서 저장소. Systems Engineer는 live/runtime/deploy 문서를 유지하고, Developer는 구현/설계 문서를 유지할 수 있다.
- `memory/knowledge/index.md`: Manager만 수정

### 도구 수준 제약 (이미 적용 중)

- Developer: Write, Edit 도구 사용 가능
- Systems Engineer: Write, Edit 도구 사용 가능
...
| Systems Engineer | opus | 전체 | runtime/deploy/cloud 분석 + 로컬 자동화 담당 |
...
- **시스템 변경 정책 관리**: `systems-engineer`가 원격 시스템 변경을 하려면 `team/systems-engineer.md` + `memory/knowledge/docs/change-authority.md`가 근거가 되어야 한다. 문서에 없거나 애매하면 Manager가 사용자 합의를 받아 문서를 갱신한다.
...
| user_notice | 비차단 user 알림 | "scope 축소, 계속 진행" |
...
| need_input | 결정이 Manager 범위면 응답. `guided`면 필요 시 유저 확인, `ralph`면 Manager가 판단 후 계속 진행. monitor의 `plan mode 판단 필요` 알림이면 해당 agent pane 최근 출력과 현재 task/report를 읽고 승인 대기인지 단순 설계 단계인지 판단 |
...
# 랄프 notify-only 알림
bash scripts/user-notify.sh {project} "scope 축소" "외부 권한 부족으로 local 검증 가능한 범위로 축소" info
```

- `to=user`일 때 tmux 전달 없이 message.log에만 기록 (유저는 tmux 윈도우가 없으므로)
- 대시보드가 message.log에서 `kind=escalation|need_input|user_notice` + `receiver=user`를 필터하여 표시
- Slack webhook과 병행 사용 가능 (대시보드 + Slack 이중 채널)
- `작업 루프 = ralph`인 프로젝트에서는 Manager → user 자동 메시지에 `need_input` / `escalation` 대신 `user_notice`를 사용한다.
