...
- priority: urgent는 monitor.sh가 `[URGENT mailbox]` 접두어로 표시한다.
- 메시지는 전달 후 즉시 삭제. 감사 로그(`memory/manager/logs/mailbox-audit.log`)가 이력을 대체한다.
...
monitor.sh가 tmux send-keys로 전달하는 알림 (전체 내용 포함):

```
[mailbox] {from} → {to} | {kind}
제목: {subject}
내용: {content}

[URGENT mailbox] {from} → {to} | {kind}
제목: {subject}
내용: {content}
```
...
- 자동 reboot 시도 (최대 3회)
- mailbox 전달은 불가하므로 (수신자가 없음) 로그에만 기록
...
## 13. Claude Code vs Codex CLI 호환성 매트릭스
...
Dual 모드에서 Codex CLI 측은 CLI 플래그 기반 제어가 불가하므로, 부팅 메시지와 mailbox 프로토콜에만 의존한다.
