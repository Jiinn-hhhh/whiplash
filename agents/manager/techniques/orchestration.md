...
- dual-dispatch는 양쪽 윈도우에 동일한 태스크를 전달한다.
...
- `memory/{role}/`: 에이전트별 개인 메모 공간. 각자 태스크 완료 시 핵심을 기록한다.
...
- **시스템 변경 정책 관리**: `systems-engineer`가 원격 시스템 변경을 하려면 `team/systems-engineer.md`가 근거가 되어야 한다. 문서에 없거나 애매하면 Manager가 사용자 합의를 받아 문서를 갱신한다.
...
| memory/{role}/에 쓰기 | 각 에이전트가 자기 memory 디렉토리만 수정 |

원칙:

- `workspace/shared/`의 기존 파일은 **Manager만** 수정
