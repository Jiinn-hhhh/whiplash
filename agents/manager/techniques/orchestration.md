...
각 에이전트는 독립된 git worktree에서 작업한다. 같은 파일을 동시에 수정하는 충돌을 원천 방지한다.

### 동작

- **부팅 시**: `boot_single_agent()`가 `projects/{name}/worktrees/{agent-id}/` 경로에 worktree를 자동 생성한다.
- **작업 중**: 에이전트는 worktree 내에서 독립적으로 커밋할 수 있다. 브랜치명: `whiplash/{project}/{agent-id}`.
- **종료 시**: `kill-agent`나 `shutdown`이 squash merge 후 worktree를 제거한다.
- **실패 시**: worktree 생성 실패 시 메인 디렉토리에서 작업 (기존 동작 유지).

### Merge 전략

- 에이전트 종료(kill-agent) 시 자동 squash merge 시도.
- merge 충돌 시 수동 merge 필요 — 브랜치와 worktree 경로가 로그에 남는다.
- Manager가 충돌 해결을 Developer에게 위임하거나 유저에게 에스컬레이션한다.
...
projects/{name}/
  worktrees/
    researcher/        # researcher 에이전트 worktree
    developer/         # developer 에이전트 worktree
    dev-hotfix/        # 동적 스폰된 에이전트 worktree
```

### Manager 예외

Manager는 worktree를 사용하지 않는다 (메인 디렉토리에서 오케스트레이션). boot-manager는 worktree 생성을 건너뛴다.
...
message.sh가 수신자의 tmux 윈도우에 `load-buffer` + `paste-buffer`로 직접 전달한다. 파일 중개 없음.
