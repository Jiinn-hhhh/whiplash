...
- trivial 예외가 아니면, 직접 구현 전에 먼저 서브에이전트를 활용한다.
- 기본 모델은 `map -> verify/debug -> implement -> review -> top-level synthesize`다.
- 공식 코드 변경, 테스트 해석, 최종 보고 책임은 항상 Developer에게 있다.
- **자율 구성**: 아래의 specialist 패턴은 참고용 출발점이다. 기존 specialist가 작업에 맞으면 활용하고, 파일별/모듈별 병렬 코딩처럼 specialist 역할 없이 범용 에이전트를 자유롭게 팀 구성하는 것도 허용한다. 어떤 조합이 효율적인지는 Developer가 판단한다.

### subagent 생략 기준

아래 중 하나라도 해당하면 subagent 없이 직접 처리해도 된다:
- 변경 파일 1-3개, 변경 줄 수 ~50줄 이내의 명확한 수정
- 기계적 수정 (오타, import, 상수, config, 단순 버그픽스)
- Developer가 영향 범위를 높은 확신으로 파악하고 있는 경우

확신이 낮거나 아키텍처 변경이면 `code-mapper` 또는 적절한 specialist를 먼저 호출한다.
