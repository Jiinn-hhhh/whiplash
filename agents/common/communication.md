...
- 종료된 토론은 정리한다 (교훈 추출 후 필요 없으면 삭제 가능).
...
기존 소통(토론, 공지)은 구조화된 문서 기반. 실시간 알림은 이를 보완하는 즉시 전달 채널이다.
...
| discussions/ | 토론 | 구조화된 토론 문서 | 종료 후 정리 |
