...
whiplash/
  ...
  workspace/                 # 런타임 작업 공간 (휘발, gitignored)
    shared/                  #   진행 중인 토론, 회의, 공지
    teams/{team-name}/       #   팀별 내부 작업 공간
  memory/                    # 축적된 상태 (누적, gitignored)
    {role}/                  #   에이전트 개인 메모
    knowledge/               #   공유 지식
      ...
      archives/              #     순환된 비활성 교훈
      index.md               #     지식 지도 (~100줄)
  reports/                   # 사용자 열람용 문서 (gitignored)
```

상세: [communication.md](communication.md) §1
...
새 에이전트가 작업을 시작할 때:
1. 이 파일(`common/README.md`)을 읽는다.
2. `memory/knowledge/index.md`를 읽어 배경 지식을 파악한다.
3. 자기 에이전트 폴더의 `profile.md`를 읽는다.
4. 작업을 시작한다.
...
| 4-folder 분리 | 프레임워크 설계 | agents/(불변) + workspace/(휘발) + memory/(누적) + reports/(출력) |
