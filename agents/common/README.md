...
whiplash/
  ...
  projects/                  # 프로젝트별 런타임 데이터 (가변, gitignored)
    {project-name}/
      ...
      memory/                #   축적된 상태
        discussion/          #     handoff.md (discussion → manager)
...
- `systems-engineer`는 `team/systems-engineer.md`를 기준으로 원격 시스템 변경 여부를 결정한다.
...
- **Layer 2 (작업 시작 시)**: memory/{role}/(개인 메모), 해당 techniques/*.md, domain context — 첫 태스크 수신 시 읽는다.
- **Layer 3 (필요 시)**: project-context.md, domain/{role}.md, team/{role}.md — 해당 정보가 필요할 때만 읽는다.
...
**Layer 2 — 작업 시작 시**
4. `memory/{role}/` 개인 메모를 읽는다 — 이전 세션 메모 (있으면).
...
10. 팀 공간, 도메인 문서 등 — 필요한 것만 on-demand로 읽는다.
